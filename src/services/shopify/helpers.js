...
module.exports.installUrl = async (shopUrl, db) => {
  ...
  const state = nonce();
  const redirectUri = `${DOMAIN}/install/callback`;
  const url =
    `https://${shopUrl}` +
 ...
 * Get Connection
 */
module.exports.getConnection = async (shopId, db) => {
  const Auth = db.model(authModel.name);
  const [auth] = await Auth.find({ shopId, accessToken: { $ne: null } }, null, {
    sort: { created: 1 },
    limit: 1,
  });
  ...
  throw new Error(`No valid auth for Shop ID [${shopId}]`);
};
